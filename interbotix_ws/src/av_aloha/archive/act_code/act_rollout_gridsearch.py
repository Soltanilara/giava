...
def run_single_rollout(
    ...
    with torch.inference_mode():
        for step in range(num_steps):
            ...
            ## Same device fix as act_rollout_experiment.select_action_from_policy:
            ## the migrated v0.6.0 preprocessor carries a device_processor
            ## serialized at MIGRATION time (cpu), so it moves the batch off the
            ## GPU the model lives on.  ACT then samples its inference latent
            ## onto observation.state's device and the mismatch surfaces inside
            ## the encoder.  Re-pin to the model's own device after preprocessing.
            _batch = preprocessor(obs)
            _dev = next(policy.parameters()).device
            _batch = {k: (v.to(_dev) if torch.is_tensor(v) else v)
                      for k, v in _batch.items()}
            output = policy.select_action(_batch)

            ## UNNORMALIZE.  Same omission as act_rollout_experiment.py: the
            ## postprocessor is loaded by build_policy and was never applied,
            ## so the raw normalized action (~mean 0, std 1) was commanded to
            ## the servos as joint radians.  Before lerobot v0.6.0 the
            ## normalization lived inside the model and select_action already
            ## returned radians, which is why this only broke after migration.
            _post = postprocessor({"action": output})
            output = (_post.get("action", _post.get("actions"))
                      if isinstance(_post, dict) else _post)
...
def main():
    ...
    parser.add_argument("--yolo-weights", type=str, default=None,
                        help="segmenter weights; auto-resolved if omitted")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    rospy.init_node("act_rollout_gridsearch", anonymous=True)

    ## The segmenter that produced the training masks.  Path was hardcoded to
    ## dataset/... and the transfer_flower runs have since moved under
    ## old_dataset/, so it is resolved rather than assumed -- and the failure
    ## is explicit, because a silently missing segmenter would feed the policy
    ## empty masks and look like a bad checkpoint.
    seg_weights = Path(args.yolo_weights) if args.yolo_weights else None
    if seg_weights is None:
        here = Path(__file__).resolve().parent.parent      # data_collection_scripts
        for cand in (here / "old_dataset/lerobot/transfer_flower_mask/best_weights_6_2.pt",
                     here / "dataset/lerobot/transfer_flower_mask/best_weights_6_2.pt"):
            if cand.exists():
                seg_weights = cand
                break
    if seg_weights is None or not Path(seg_weights).exists():
        raise SystemExit(
            "could not find the YOLO segmenter weights (best_weights_6_2.pt). "
            "Pass --yolo-weights <path>.")
    print(f"[yolo] {seg_weights}")
